...
function (Controller,Formatter, Filter, MessageToast, MessageBox, JSONModel, History, Dialog, Button, Fragment) {
    "use strict";
    ...
    return Controller.extend("zappproxy.controller.DeviationApprovalMaster", {
        onInit: function () {
			  ...
			  if(!this._oDialogDeviationApproval){
				this._oDialogDeviationApproval = sap.ui.xmlfragment("zappproxy.fragments.DeviationApproval", this);
			 ...
			 jQuery.sap.syncStyleClass("sapUiSizeCompact", this.getView(), this._oDialog);
			  ...
			  var that=this;
			 // this._UserID = sap.unshell.Container.getService("UserInfo").getId();
		...
		OnGo:function(oEvent){
			...
			if(PONo.getValue() === "" || idUserId.getValue() === ""){
				...
				oModel.read("/ValidateUserSet('"+PUserId+"')",{
					 success:function(odata,oResponse){
                              if(odata.Valid === true){
								oModel.read("/POProxyMatchSet(PO_NO='" + PONo.getValue() + "',UserID='" + PUserId + "')",{
									success:function(odata,oResponse){
										if(odata.Valid === true){
                                          oModel.read("/POProxyDetailsSet(PO_NO='" + PONo.getValue() + "',UserID='" + PUserId + "')",{
											success:function(odata,oResponse){
											...
											MessageBox.error("This PO no is not valid for this USERID");
										}
		...
		onSelectionChange:function(oEvent){
			...
			var PoNo = oEvent.getParameters().listItem.getTitle();
		...
		onListItemPress:function(oEvent){
			...
			var Sid = this.getView().sId;
			var ProxyUserId = this.getView().byId("txtPUserID").setText(objEdit.UserID);
			this.getRouter().navTo("DeviationApprovalDetail",{
				PO_No: objEdit.PO_NO,
				Sid: Sid,
		...
		onSearch:function(oEvent){
			...
			var oFilter = new sap.ui.model.Filter({
				Filters:[new sap.ui.model.Filter("PO_No",sap.ui.model.FilterOperator.Contains, sQuery),]
			});
		...
		handleConfirm:function(oEvent){
			...
			if(query.length > 0){
				var oFilter = new  sap.ui.model.Filter({
					filters:[new sap.ui.model.Filter("PO_Status", sap.ui.model.FilterOperator.EQ, query[0].getText()),]
				});
